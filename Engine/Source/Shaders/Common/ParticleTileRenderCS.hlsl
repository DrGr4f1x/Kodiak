...
//
// Adapted from ParticleTileRenderCS.hlsl in Microsoft's Miniengine sample
...
void BlendHighRes(inout float4x4 Quad, ParticleScreenData Particle, float2 PixelCoord, float4 Mask = 1)
{
	...
	SamplerState Sampler = PointBorderSampler;
	float LevelBias = gMipBias;
#else
	SamplerState Sampler = LinearBorderSampler;
	float LevelBias = 0.0;
#endif
...
{
	float2 UV = (PixelCoord - Particle.Corner) * Particle.RcpSize;
	float4 Color = SampleParticleColor(Particle, LinearBorderSampler, UV, 1.0);
#ifdef DEBUG_LOW_RES
	Color.g *= 0.5;
#endif
...
float4x4 RenderParticles(uint2 TileCoord, uint2 ST, uint NumParticles, uint HitMaskStart, uint BinStart)
{
	...
	float4 Depths = g_InputDepthBuffer.Gather(PointSampler, (ST + 1) * gRcpBufferDim);
#endif
